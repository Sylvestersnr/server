...
{
    public:
        SqlPreparedRequest(int nIndex, std::unique_ptr<SqlStmtParameters> arg);

        bool Execute(SqlConnection *conn);

    private:
        const int m_nIndex;
        std::unique_ptr<SqlStmtParameters> m_param;
};
...
class SqlResultQueue : public LockedQueue<MaNGOS::IQueryCallback*>
{
    public:
